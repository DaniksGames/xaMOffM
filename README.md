...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, sans-serif; }
        ...
        .menu-btn:active { background: var(--contact-hover); }
        
        .side-menu { position: fixed; top: 0; left: -280px; width: 280px; height: 100vh; background: var(--sidebar-bg); z-index: 200; transition: left 0.3s; box-shadow: 2px 0 12px rgba(0,0,0,0.15); }
        ...
        .side-menu-item:active { background: var(--contact-hover); }
        ...
        .chat-item { display: flex; align-items: center; gap: 12px; padding: 12px 16px; cursor: pointer; border-bottom: 1px solid var(--input-border); }
        ...
        .message { display: flex; max-width: 85%; padding: 4px; border-radius: 12px; position: relative; transition: background 0.15s; cursor: pointer; user-select: none; -webkit-tap-highlight-color: transparent; }
        ...
        .selection-bar { position: fixed; bottom: 80px; left: 16px; right: 16px; background: var(--header-bg); color: white; padding: 12px 20px; border-radius: 40px; display: none; align-items: center; justify-content: space-between; z-index: 150; box-shadow: 0 4px 20px rgba(0,0,0,0.3); }
        .selection-bar.visible { display: flex; animation: slideIn 0.2s; }
        ...
        .modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 300; display: none; align-items: center; justify-content: center; }
        ...
        .modal-content input, .modal-content textarea { width: 100%; padding: 12px; margin: 8px 0; border-radius: 20px; border: 1px solid var(--input-border); background: var(--input-bg); color: var(--other-text); font-size: 1rem; }
        ...
        .recording-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); backdrop-filter: blur(10px); z-index: 400; display: none; flex-direction: column; align-items: center; justify-content: center; }
        ...
        .recording-timer { position: absolute; top: 20px; left: 20px; color: white; font-size: 1.2rem; background: rgba(0,0,0,0.5); padding: 8px 16px; border-radius: 30px; }
        ...
        .admin-user-name { font-weight: bold; margin-bottom: 8px; }
        .admin-user-detail { font-size: 0.8rem; opacity: 0.8; margin-bottom: 4px; }
        .admin-user-actions { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px; }
        .admin-user-actions button { padding: 6px 12px; border-radius: 20px; border: none; cursor: pointer; font-size: 0.7rem; }
        
        .toast { position: fixed; bottom: 100px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.8); color: white; padding: 8px 16px; border-radius: 30px; font-size: 0.8rem; z-index: 500; display: none; }
        .highlight-message { animation: fadeIn 0.3s; }
...
<div class="app-container" id="appContainer">
    ...
    <div id="addContactModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <input type="text" id="searchContactInput" placeholder="Поиск по нику...">
    ...
    <div id="groupInfoModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <button id="leaveGroupBtn" style="background:#ef4444;">Покинуть группу</button>
            <button id="deleteGroupBtn" style="background:#dc2626; display:none;">Удалить группу</button>
    ...
    container.innerHTML = chats.map(c => `
        <div class="chat-item" onclick="openChat('${c.id}', '${escapeHtml(c.name).replace(/'/g,"\\'")}', ${c.isGroup})">
            <div style="position:relative;">
                <img src="${getAvatarUrl(c.name, c.avatar)}" style="width:52px;height:52px;border-radius:50%;object-fit:cover;">
...
function loadMessages() {
    ...
    area.innerHTML = '<div class="system-message"><div class="bubble">Загрузка...</div></div>';
...
function renderMessage(msgId, msg, path) {
    ...
    // Клик для выхода из режима выделения (нажатие на пустую область)
    if(!selectionMode) {
...
async function forwardSelectedMessages() {
    ...
    container.innerHTML = targets.map(t => `<div class="forward-contact" onclick="forwardToChat('${t.id}', ${t.isGroup})"><img src="${getAvatarUrl(t.name, t.avatar)}"><div>${escapeHtml(t.name)}</div></div>`).join('');
...
async function startRecording(type) {
    ...
    if(type === 'circle') {
        ...
        try {
            ...
            showToast('Ошибка доступа к камере. Проверьте разрешения.');
        ...
        try {
            ...
            showToast('Ошибка доступа к микрофону. Проверьте разрешения.');
            overlay.classList.remove('visible');
            return;
        }
    }
...
function drawWaveform() {
    ...
    for(let i = 0; i < bars.length && i < buffer.length; i++) {
        ...
        const height = Math.max(5, Math.min(80, value / 3));
...
function stopRecordingAndSend() {
    ...
    reader.onload = e => {
        sendMessage('', recordType === 'circle' ? 'video' : 'audio', e.target.result, null, null, recordType === 'circle');
    };
...
document.getElementById('deleteRecBtn').onclick = () => {
    if(mediaRecorder && mediaRecorder.state === 'recording') {
        mediaRecorder.stop();
    }
    if(recordStream) recordStream.getTracks().forEach(t => t.stop());
...
document.getElementById('sendRecBtn').onclick = () => {
...
document.getElementById('attachContactBtn').onclick = () => {
    document.getElementById('attachMenu').classList.remove('visible');
...
// Контакты и группы
async function showAddContactModal(isSelectMode = false) {
    ...
    input.oninput = async () => {
        ...
        const snap = await db.ref('users').once('value');
        const existing = new Set(chats.filter(c=>!c.isGroup).map(c=>c.id));
        let html = '';
        for(let id in snap.val()) {
            const u = snap.val()[id];
            if(u.name.toLowerCase().includes(q) && u.name !== currentUserName && !u.blocked && !existing.has(id)) {
                html += `<div class="forward-contact" onclick="addContact('${id}', '${escapeHtml(u.name)}')"><img src="${getAvatarUrl(u.name, u.avatarUrl)}"><div>${escapeHtml(u.name)}</div></div>`;
            }
        }
        results.innerHTML = html || '<div style="padding:16px;text-align:center;">Не найдено</div>';
    };
}
...
window.toggleMember = (id) => { 
    const cb = document.getElementById(`member_${id}`); 
...
document.getElementById('saveProfileBtn').onclick = async () => {
    ...
    if(userId === currentUserId) {
        ...
        if(newName && newName !== currentUserName) {
            ...
            await db.ref('users/'+currentUserId).update({ bio: newBio });
        }
...
window.adminEditUser = async (userId) => {
    ...
    if(newPass && newPass !== user.password) {
        await db.ref('users/'+userId).update({ password: newPass });
    }
    const newBio = prompt('Новое описание:', user.bio || '');
    if(newBio !== undefined) {
        await db.ref('users/'+userId).update({ bio: newBio });
    }
    ...
    if(confirm('Очистить все сообщения этого пользователя во всех чатах?')) {
        ...
        for(let chatId in privateSnap.val()) {
            if(chatId.includes(userId)) {
                await db.ref(`${privatePath}/${chatId}`).remove();
            }
        }
        ...
        for(let groupId in groupSnap.val()) {
            ...
            for(let msgId in msgs) {
                if(msgs[msgId].userId === userId) {
                    await db.ref(`group_messages/${groupId}/${msgId}`).remove();
                }
            }
        }
...
window.adminDeleteUser = async (userId) => {
    if(confirm('Удалить пользователя навсегда? Все его сообщения будут удалены.')) {
...
document.getElementById('adminMenuItem').onclick = () => { 
    showAdminPanel(); 
    document.getElementById('sideMenu').classList.remove('open'); 
    document.getElementById('menuOverlay').classList.remove('visible'); 
};
document.getElementById('logoutMenuItem').onclick = () => { 
    db.ref('users/'+currentUserId).update({ online: false }); 
    localStorage.clear(); 
    location.reload(); 
};
document.getElementById('fabBtn').onclick = () => {
    ...
    modal.innerHTML = `
        <div class="modal-content">
            <h3>Действия</h3>
            <button id="createGroupOpt">👥 Создать группу</button>
            <button id="addContactOpt">➕ Добавить контакт</button>
...
async function auth() {
    ...
    try {
        ...
        document.getElementById('authError').innerText = 'Ошибка: ' + e.message;
        btn.disabled = false;
        btn.innerText = 'Войти';
    }
}
